calcDistances <- function(melodies, melodiesNames, authors, eigensList, docTermMatrix) {
  ...
  if(is.null(eigensList))  len <- length(melodiesNames)
  else len<-length(eigensList)
  ...
  for(i in 1:len) {
    for(j in 1:len) {
      ...
      else  d <- distanceMatrix[i,j] <- distanceMatrix[j,i] <- lsa::cosine(docTermMatrix[i,], docTermMatrix[j,])
